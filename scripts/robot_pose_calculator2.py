...
class RobotPoseCalculator(Node):
    def __init__(self):
        ...
        # Add TF broadcaster to publish the connecting transform
    ...
    def calculate_and_publish_pose(self):
        try:
            ...
            # NEW: Publish TF transform to bridge the two TF trees
            self.publish_tf_bridge(robot_pose, camera_to_object.header.stamp)
    ...
    def publish_tf_bridge(self, robot_pose_matrix, timestamp):
        """
        Publish TF transform to bridge the two TF trees
        This connects camera_link (from camera TF tree) to base_link (from robot TF tree)
        """
        ...
        transform_stamped.header.frame_id = 'camera_link'  # Parent frame (camera tree)
        transform_stamped.child_frame_id = 'base_link'     # Child frame (robot tree)
        
        # Transform data
        transform_stamped.transform = self.matrix_to_transform(robot_pose_matrix)
        
        # Broadcast the transform
        self.tf_broadcaster.sendTransform(transform_stamped)
        
        self.get_logger().debug(
            f'Published TF bridge: camera_link -> base_link'
        )
